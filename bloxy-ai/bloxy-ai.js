...
async function sendMessage(){
    ...
    try {
       ...
       =
        ...
        if(!response.ok){
            throw new Error("ERROR: Try Again In 1 Minute Or More (Up To 48 Hours)");
        }

        const data = await response.json();
        const reply = data.candidates?.[0]?.content?.parts?.[0]?.text || "ERROR: Try Again In 1 Minute Or More (Up To 48 Hours)";
